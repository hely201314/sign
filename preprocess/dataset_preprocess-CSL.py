...
def csv2dict(dataset_root, anno_path):
    ...
    for file_idx, file_info in tqdm(enumerate(inputs_list), total=len(inputs_list)):
        ...
        num_frames = len(glob.glob(f"{name}/*.png"))
        info_dict[file_idx] = {
            'fileid': name,
            'folder': name.split('/')[-1],
            'signer': 'unknown',
...
def resize_dataset(video_idx, dsize, info_dict, target_path):
    ...
    img_list = glob.glob(f"{info['fileid']}/*.png")
    if len(img_list) == len(glob.glob(f"{target_path}/features/fullFrame-{dsize}/{info['folder']}/*.png")):
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='CSL',
                        help='save prefix')
    parser.add_argument('--dataset-root', type=str, default='/remote-home/cs_cs_heli/AdaptSign/preprocess/',
                        help='path to the dataset')
    parser.add_argument('--target-path', type=str, default='/remote-home/cs_cs_heli/AdaptSign/preprocess/CSL_Continuous_Resized',

...
class BaseFeeder(data.Dataset):
    ...
    def read_video(self, index):
        ...
        #add_key_frame
        key_index_path =os.path.join(self.prefix, "img_index_" + self.mode+".json")
        with open(key_index_path, 'r') as f:
            img_index = json.load(f)
        results = img_index[fi['fileid']]
        results = results[0]
        img_list = [img_list[i-1] for i in results]

...
# add
# from kan import KAN
# from mlp_att import Block
# from selatt import Block
from sparseStm import sparse
# from selatt import EfficientAdditiveAttnetion
# from layer import GC
from adater_ import Adapter2

from testaw import CausalSelfAttention_TSSA2
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, checkpointing=False):
        ...
        self.S_Adapter = Adapter2(d_model)
        #
        ...
        # add
        # self.s_ada = Adapter2(d_model)
        # self.mlp_ada = Adapter2(d_model)
    ...
    def attention(self, x: torch.Tensor):
        ...
        return self.attn(x, torch.cat((x, self.prefix_embedding_k.repeat(1, x.shape[1], 1)), 0),
                         torch.cat((x, self.prefix_embedding_v.repeat(1, x.shape[1], 1)), 0), need_weights=False,
                         attn_mask=self.attn_mask)[0]

    # def forward(self, x: torch.Tensor):
    def forward(self, x: torch.Tensor, T):
        if self.checkpointing:
            x = x + checkpoint(self.attention, checkpoint(self.ln_1, x)) + self.S_Adapter(x, T)
            # x = x + checkpoint(self.attention, checkpoint(self.ln_1,x)) + self.s_ada(x,T)
            # x = x + checkpoint(self.attention, checkpoint(self.ln_1,x))
        else:
            x = x + self.attention(self.ln_1(x)) + self.S_Adapter(x, T)
            # x = x + self.attention(self.ln_1(x)) + self.s_ada(x,T)
            # x = x + self.attention(self.ln_1(x))
        # x = x + self.S_Adapter(self.attention(self.ln_1(x)))
        # x = x + self.mlp(self.ln_2(x))
        if self.checkpointing:
            x = x + checkpoint(self.mlp, checkpoint(self.ln_2, x)) + self.MLP_Adapter(x)
            # x = x + checkpoint(self.mlp, checkpoint(self.ln_2,x)) + self.mlp_ada(x,T)
            # x = x + checkpoint(self.mlp, checkpoint(self.ln_2,x))
        else:
            x = x + self.mlp(self.ln_2(x)) + self.MLP_Adapter(x)
            # x = x + self.mlp(self.ln_2(x)) + self.mlp_ada(x,T)
            # x = x + self.mlp(self.ln_2(x))
...
class Adapter(nn.Module):
    def __init__(self, d_model: int, skip_connect=True):
        super().__init__()
        r = 1 / 4
        # self.mlp = KAN([d_model,32,d_model])
        self.mlp = nn.Sequential(OrderedDict([
...
class AggregationBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cls):
        cls = cls + self.attention(self.ln_1(torch.cat((cls, x), 0)))
        cls = cls + self.mlp(self.ln_2(cls))
        # cls = (2 - self.temporal_ada_weight) * cls + self.temporal_ada_weight * self.attention(
        #     self.ln_1(torch.cat((cls, x), 0)))
        #
        # cls = cls + self.mlp(self.ln_2(cls))
        return cls
...
class UnfoldTemporalWindows(nn.Module):
    ...
    def forward(self, x, T):
        ...
        NT, C, P = x.shape#(448,768,196)
        x = x.view(-1, T, C, P).permute(0, 2, 1, 3)  # n,t,c,p->n,c,t,p(2,768,224,196)
        x = self.unfold(x)  # (N, C*Window_Size, T, P)(2,3840,43904)
        # Permute extra channels from window size to the graph dimension; -1 for number of windows
        x = x.view(-1, C, self.window_size, T, P).permute(0, 3, 1, 2, 4).reshape(NT, C, -1)  # (NT)C(SP)(448,768,980)
        # (N, C*Window_Size, T, P)->(N,C,Window_Size,T,P)->(N,T,C,Window_Size,P)->(NT)C(SP)
        return x
...
class Correlation_Module(nn.Module):
    ...
    def forward(self, x, upfold):
        L, N, D = x.shape
        import math
        affinities = torch.einsum('lnd,ond->lon', x, upfold) / math.sqrt(D)#(1,980,448,)
        features = torch.einsum('lon,ond->lnd', F.sigmoid(affinities) - 0.5, upfold)#(1,448,768,)
        #x = (1,448,768)
        return features
...
class TemporalAggregationBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.spa = sparse(enc_in=d_model)
        self.spa_weight = nn.Parameter(torch.zeros(1), requires_grad=True)

    def attention(self, x: torch.Tensor, T):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        x_upfold = self.upfold(x[1:].permute(1, 2, 0), T).permute(2, 0, 1)  # LND -> NDL -> LND(980,448,768)
        #x(197,448,768)
        # add

        x1 = self.spa(x.permute(1, 0, 2)).permute(1, 0, 2)#(197,448,768)
        x1 = torch.mean(x1, dim=0).unsqueeze(0)#(1,448,768)
        # x_upfold1 = self.spa(x_upfold.permute(1,0,2)).permute(1,0,2)
        # x_upfold1 = torch.mean(x_upfold1, dim=0).unsqueeze(0)
        x_att_ufold = self.attn(x[:1], x_upfold)#(1,448,768)
        # x1 x= 197 288(-) 768 x_att_ufold = 1,288,768
        return x_att_ufold + x1 * self.spa_weight  # cls token attends to others
        # return x1
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, checkpointing=True):
        ...
        # add
        #2
        # self.aggblocks = nn.Sequential(*[Block(width, heads) for _ in range(layers)])
        # self.aggblocks = nn.Sequential(*[CausalSelfAttention_TSSA2(width, heads) for _ in range(layers)])
    ...
    def forward(self, x: torch.Tensor, T=None):
        ...
        #2
            # if self.checkpointing:
            #     query = checkpoint(self.aggblocks[i], x, query)  # reducing consuming
            # else:
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        # add
        # self.graphcon = GC(width)

    def forward(self, x: torch.Tensor, T):
        # with torch.no_grad():T = 172
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND#197,344,768
        ...
        #2
        x = self.ln_post(x[:, 0, :]) * self.ada_weight[0] + self.ln_post_cls(new_cls.permute(1, 0, 2))[:, 0] * \
            self.ada_weight[1]
        # x = self.ln_post(x[:, 0, :])
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # vision_layers = 12 vision_width 768  vision_patch_size 16 embed_dim 512
        super().__init__()

...
device_id = 0
dataset = 'phoenix2014'
prefix = '/remote-home/cs_cs_heli/data/phoenix2014/phoenix-2014-multisigner'
...
# model.conv2d.visual.transformer.resblocks[11].MLP_Adapter.register_forward_hook(forward_hook)
# model.conv2d.visual.transformer.resblocks[11].MLP_Adapter.register_backward_hook(backward_hook)
# model.conv2d.visual.transformer.aggblocks[11].ln_1.register_forward_hook(forward_hook)#grads[:, 2:, :]
# model.conv2d.visual.transformer.aggblocks[11].ln_1.register_backward_hook(backward_hook)#grads[:, 2:, :]

model.conv2d.visual.transformer.taggblocks.ln_1.register_forward_hook(forward_hook)
model.conv2d.visual.transformer.taggblocks.ln_1.register_backward_hook(backward_hook)

def cam_show_img(img, feature_map, grads, out_dir):  # img: ntchw, feature_map: ncthw, grads: ncthw
    #feature_map and grads = (197=14*14+1,T,768)
    ...
    for i in range(T):
        ...
        img_out = (img[0,i]/2+0.5).permute(1,2,0).cpu().data.numpy()
        img_out = img_out / np.max(img_out)
        img_out = np.uint8(255 * img_out)

        # cv2.cvtColor(cam_img, cv2.COLOR_RGB2BGR, cam_img)#反转颜色
        path_cam_img = os.path.join(out_dir, f"cam_{i}.jpg")
        cv2.imwrite(path_cam_img, img_out)
...
# # generate cam
# grads = torch.stack(grad_block,1).mean(1)
# grads = grads.permute(1, 0, 2)  # (152,197,768)
# grads = grads[:, 1:, :].view(grads.shape[0], 14, 14, grads.shape[2])  # (152,14,14,768)
# grads = torch.unsqueeze(grads, dim=0)  ##(1,152,14,14,768)
# grads = grads.permute(0, 4, 1, 2, 3)  # (152,197,768)##(1,768,152,14,14)
# grads_val = grads.cpu().data.numpy()
#
#
# fmap = fmap_block[0]
# feature_map = fmap.permute(1,0,2)#(152,197,768)
# feature_map = feature_map[:,1:,:].reshape(feature_map.shape[0],14,14,feature_map.shape[2])#(152,14,14,768)
# feature_map = torch.unsqueeze(feature_map, dim=0)##(1,152,14,14,768)
# fmap =  feature_map.permute(0,4,1,2,3)#(152,197,768)##(1,152,768,14,14)
#
# # fmap = fmap_block[0].cpu().data.numpy()
# fmap = fmap.cpu().data.numpy()
#
#
# # save image
